...
int main() {
    ...
    constexpr std::array<std::string_view, 12> ARGUMENTS = {
        "predict", "--beam-size=3", "--score", "-i",    "input.txt", "-o=output.txt",
        "-it",     "jsonl",         "-m",      "model", "-a",        "0.7",
    };
    ...
    if (arguments.get<std::string>("command") != "predict" || arguments.get<std::int32_t>("beam_size") != 3 ||
        !arguments.get<bool>("score") || arguments.get<std::filesystem::path>("model") != "model" ||
    ...
    if (help.status() != kidi::cli::ParseStatus::HELP || !contains(help.output(), "usage: kidi predict") ||
        !contains(help.output(), "--beam-size N") || !contains(help.output(), "--in FILE") ||
    ...
    if (defaults.get<std::string>("input") != "-" || defaults.get<std::string>("output") != "-" ||
        defaults.get<std::string>("inp_type") != "text" || defaults.get<bool>("score")) {
