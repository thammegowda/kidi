...
std::expected<std::string, core::Error> read_plain(const std::filesystem::path& path) {
    std::ifstream input(path, std::ios::binary);
    if (!input) {
        return std::unexpected(core::Error{core::ErrorCode::IO, "cannot open tokenizer: " + path.string()});
    }
    std::string contents(std::istreambuf_iterator<char>{input}, std::istreambuf_iterator<char>{});
    if (input.bad()) {
        return std::unexpected(core::Error{core::ErrorCode::IO, "cannot read tokenizer: " + path.string()});
    }
    return contents;
}

std::expected<std::string, core::Error> read_gzip(const std::filesystem::path& path) {
    gzFile input = gzopen(path.c_str(), "rb");
    if (input == nullptr) {
        return std::unexpected(core::Error{core::ErrorCode::IO, "cannot open gzip tokenizer: " + path.string()});
    }
    ...
    if (gzclose(input) != Z_OK) {
        return std::unexpected(core::Error{core::ErrorCode::IO, "cannot close gzip tokenizer: " + path.string()});
    }
    return contents;
}
...
std::expected<Tokenizer, core::Error> Tokenizer::load(const std::filesystem::path& path) {
    if (!std::filesystem::is_regular_file(path)) {
        return std::unexpected(core::Error{core::ErrorCode::IO, "tokenizer does not exist: " + path.string()});
    }
...
std::expected<std::vector<std::int32_t>, core::Error> Tokenizer::encode(std::string_view text) const {
    auto encoding = impl_->value.encode(text, false);
    if (!encoding) {
        return std::unexpected(
            core::Error{core::ErrorCode::RUNTIME, "tokenizer encode failed: " + encoding.error().message()});
    }
    return encoding->get_ids();
}

std::expected<std::string, core::Error> Tokenizer::decode(std::span<const std::int32_t> ids) const {
    const std::vector<std::int32_t> owned_ids(ids.begin(), ids.end());
    auto text = impl_->value.decode(owned_ids, false);
    if (!text) {
        return std::unexpected(
            core::Error{core::ErrorCode::RUNTIME, "tokenizer decode failed: " + text.error().message()});
    }
